...
from agensight.tracing import (
    setup_tracing, get_tracer, start_span,
    enable_session_tracking, set_session_id
)
from agensight.integrations import instrument_openai

# Setup tracing
setup_tracing("chatbot-with-tools")
instrument_openai()
enable_session_tracking()
set_session_id("session-with-tools")
...
class Agent:
    ...
    def call_llm(self, messages):
        with start_span(tracer, f"{self.name}_call", {
            ...
            if choice.finish_reason == "tool_calls":
                ...
                arguments = eval(tool_call.function.arguments)  # safe for demo only
...
def main():
    with start_span(tracer, "multi_agent_chat", {"flow": "full"}):
